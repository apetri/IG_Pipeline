...
#define SKIP2 {my_fread(&dummy2, sizeof(int), 1, fd);}
  ...
  if(ThisTask == 0)
      ...
      for(num = 0, skip = 0; num < numfiles; num++)
	{
      ...
      // <JMK>:
      printf("Header1 size, dummy, dummy2: %d %d %d\n", ((int) sizeof(header)), dummy, dummy2);
      fflush(stdout);
      // </JMK>
	  ...
	  for(type = 0, n = 0; type < 6; type++)
	      ...
	      for(m = 0; m < header1.npartTotal[type]; m++, n++)
		{
		  ...
		  if(Slab_to_task[slab] == ThisTask)
		    {
		      y = pos[3 * n + 1] / header1.BoxSize * (Box / TileFac) + j * (Box / TileFac);
		      z = pos[3 * n + 2] / header1.BoxSize * (Box / TileFac) + k * (Box / TileFac);
