...
void initialize_darkenergy (void) {
	...
	if (ThisTask == 0) printf("ystart, nok, nbad, nrhs: %e %d %d %d\n", ystart[1], nok, nbad, nrhs);
	...
	for (i=1;i<=kount;i++)
	{
		...
		if (ThisTask == 0) printf("Eq 1: i, a (scale factor), xp (redshift), DEp (dark energy e-factor): %d --  %e %e %e\n", i, ap[i], xp[kount+1-i], DEp[1][i]);
	}
...
*/	
    ...
    if (ThisTaks == 0) printf("Finished initializing dark energy.\n");
    return;
}
...
double w(double z)
{
	...
	//w0=-1.0; wa=0.0;
	ww=w0+(z/(1.0+z))*wa; // example of a redshift-dependent dark energy equation of state parameter w(z).
	return ww;
...
{
	double yy;
	if (w0>-1.0001 && w0<-0.9999 && wa>-0.0001 && wa<0.0001) yy=1.0; // This is the cosmological constant model (in that case, the integrand above is zero and odeint makes very few steps, so the spline construction would have small wiggles, thus set to precise value). 
