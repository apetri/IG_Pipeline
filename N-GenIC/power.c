  ...
  if(ReNormalizeInputSpectrum == 0 && WhichSpectrum == 2)
    {
      ...
      // <JMK>:
      printf("My Norm of spectrum without adjustments is: %g\n", Norm);
      // </JMK>.
      Norm = 1.0;
	...
	printf("\nNormalization of spectrum in file:  TopHatSigma2 = %.12g, Sigma8 = %.12g\n", res, sqrt(res));

      Norm = Sigma8 * Sigma8 / res;

      if(ThisTask == 0 && WhichSpectrum == 2)
	printf("Normalization adjusted to  Sigma8=%g   (Normfac=%.12g)\n\n", Sigma8, Norm);

        //<JMK>:
      // Dplus = GrowthFactor(InitTime, 1.0); // original line in N-Genic, replaced by:
        Dplus_springel = GrowthFactor(InitTime, 1.0);
        
        if(ThisTask == 0)
        {
            printf("Springel's Classic Growth Factor Dplus from initial redshift till today: Dplus=%1.10g\n", Dplus_springel);
            printf("If Norm=Dplus^2: %1.10g = %1.10g, input power spectrum already scaled to initial redshift, and yielding the selected sigma8.\n", Norm, Dplus_springel*Dplus_springel);
            // Final power spectrum Delta^2 will be rescaled to starting redshift by Norm/Dplus^2.
            // If therefore Norm=Dplus^2, then input power spectrum was already scaled to initial redshift and provides the selected sigma8.
        }
        
#ifndef PREFAB_GROWTH
        Dplus = Dplus_Interface(1.0/InitTime-1.0, 1.0/1.0-1.0, Omega, OmegaLambda, 0.0, w0, wa, HubbleParam);
#endif
        // if PREFAB_GROWTH is defined, Dplus and vel_prefac_lam have been read in from parameter file of N-GenIC.
        if(ThisTask == 0)
        {
            printf("Lam's Growth Factor Dplus from initial redshift till today: Dplus=%1.10g\n", Dplus);
            printf("If Norm=Dplus^2: %1.10g = %1.10g, input power spectrum already scaled to initial redshift, and yielding the selected sigma8.\n", Norm, Dplus*Dplus);
        }
        // </JMK>.
...
double TopHatSigma2(double R)
{
  ...
  r_tophat = R;
    ...
    // <JMK>:
    // For our tabulated power spectrum, only 10^-4 relative precision is achievable for the sigma8 calculation for some reason, not Volker Springel's original 10^-8:
  // gsl_integration_qag(&F, 0, 500.0 * 1 / R, 0, 1.0e-8, WORKSIZE, GSL_INTEG_GAUSS41, workspace, &result, &abserr);
    gsl_integration_qag(&F, 0, 500.0 * 1 / R, 0, 1.0e-4, WORKSIZE, GSL_INTEG_GAUSS41, workspace, &result, &abserr);
    // </JMK>.
