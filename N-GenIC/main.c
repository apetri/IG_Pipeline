...
void displacement_fields(void)
{
  ...
  vel_prefac_springel = vel_prefac;
    
  vel_prefac=vel_prefac_lam; // comment out this line to get N-GenIC's original prefactor.
    
  if(ThisTask == 0)
  {
      printf("Springel's classic vel_prefac= %1.10g  hubble_a=%g fom=%g \n", vel_prefac_springel, hubble_a, F_Omega(InitTime));
      printf("Lam's velocity prefactor: vel_prefac=%1.10g\n", vel_prefac);
  }
