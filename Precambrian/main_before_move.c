...
header2;                               /*!< holds header for snapshot files */
...
struct io_header_1
{
  ...
  uint4byte npartTotal[6]; /*!< npart[1] gives the total number of particles in the run. If this number exceeds 2^32, the npartTotal[2] stores the result of a division of the particle number by 2^32, while npartTotal[1] holds the remainder. */
  ...
  double w0;
  double wa;
  char fill[40]; // char fill[68]; // decreased to accomodate additional variables, original was: char fill[84];                /*!< fills to 256 Bytes */              
} header, header1;
...
char ics_dir_speccomp[1000], ics_dir_simcomp[1000], Gadget_output_dir[1000]; // full master paths on power spectrum generation computer cluster (LSST) and simulation computer cluster (NYBlue), as well as location of simulation output (snapshots) of Gadget-2 on simulation computer cluster (NYBlue).
...
char command[200], job_description_filename[1000], jobstammL[200], jobstammP[200], BG_type[200], directory_script_filename[1000]; // string containing Condor job description filename and NYBlue job description filenames.
FILE *CAMB_condor_file, *directory_script_file; // pointer to Condor job description file.
...
// 2. Run Condor script which runs CAMB on LSST cluster
// 3. Run mode=2 (converts CAMB output for ingestion by N-GenIC).
// 4. Run mode=3 (generates parameter files for N-GenIC and Gadget-2 (all cosmological parameter combinations) to selects which combinations will be run (generates Blue Gene job description files).
// 5. Submit N-GenIC to Blue Gene
// 6. Submit Gadget-2 to Blue Gene
// 7. Optionally repeat steps 4.--6. if want to add different box sizes later (which were not included in first pass).
...
// Execution modes are directly chosen by providing an argument. Additional parameters are selectable in this file below.

 printf("Sizes: short, long, long long, int, uns int, float, double: %d %d %d %d %d %d %d\n", ((int) sizeof(short)), ((int) sizeof(long)), ((int) sizeof(long long)), ((int) sizeof(int)), ((int) sizeof(unsigned int)), ((int) sizeof(float)), ((int) sizeof(double)));
 printf("Size of header1: %d %d\n", ((int) sizeof(header1)), ((int) sizeof(header1.w0)));
 printf("Size of header2: %d %d\n", ((int) sizeof(header2)), ((int) sizeof(header2.w0)));
...
submission_style=1; // select 1, 2 (recommended default), or 3 here. Affects only mode=4 (submission script generation).
// 1: all, 2: cross jobs only, 3: fiducial model only (many random seeds).
...
sprintf(series, "r"); // simulation series name (typically one lower case letter).
...
Nboxsize=6; // number of different boxsizes to be evaluated (more can be added later easily).
...
Nom=1; // Number of different parameter values of OMEGA MATTER to be investigated; make sure number equals number of different parameter values below.
...
Nw0=1; // make sure number equals number of different parameter values below.
...
// sigma_8:
Ns8=1; // make sure number equals number of different parameter values below.
...
Nseed=1; // make sure number equals number of different parameter values below.
...
sprintf(NGenIC_dir_L, "/gpfs/home2/jank/Documents/N-GenIC/noGSL-preDplus-N-GenIC"); // path to version for Blue Gene/L
sprintf(NGenIC_dir_P, "/gpfs/home2/jank/Documents/N-GenIC/preDplus-N-GenIC"); // path to version for Blue Gene/P
sprintf(Gadget_dir_L,"/gpfs/home2/jank/Documents/Gadget/noGSL-w-Gadget-2.0.3/Gadget2");
sprintf(Gadget_dir_P,"/gpfs/home2/jank/Documents/Gadget/Octopus-w-Gadget-2.0.3/Gadget2");
// Executables:
sprintf(CAMB_exec, "camb");
sprintf(NGenIC_exec, "N-GenIC");
sprintf(Gadget_exec, "Gadget2");
// Master directories for initial conditions:
sprintf(ics_dir_simcomp, "/gpfs/scratch3/jank/Storage/sims/ics");
sprintf(ics_dir_speccomp, "%s", ics_dir_simcomp); // For LSST cluster: sprintf(ics_dir_speccomp, "/data/jank/Storage/ics");
sprintf(Gadget_output_dir, "/gpfs/scratch3/jank/Storage/sims/snapshots");
// Folders:
	...
	else if (mode==3 || mode==4) // generate all N-GenIC and Gadget-2 parameter files, and selected Blue Gene job description files and submission shell scripts. 
	{
		...
		for (i_boxsize=0; i_boxsize<Nboxsize; i_boxsize++)
		{
		  ...
		  soft=7.5*512.0*boxsize[i_boxsize]/(200.0*part); // This is scaled formula from standard simulation (similar softening to Millenium simulation, Bologna simulation, etc.).
			...
			if (mode==4)
			{
				if (submission_style==1 || (submission_style==2 && (i_OBh2!=0)+(i_OM!=0)+(i_OL!=0)+(i_w0!=0)+(i_wa!=0)+(i_ns!=0)+(i_s8!=0)<=1) || (submission_style==3 && (i_OBh2!=0)+(i_OM!=0)+(i_OL!=0)+(i_w0!=0)+(i_wa!=0)+(i_ns!=0)+(i_s8!=0)==0))
				{
					jobs++;
...
void write_NGenIC_parameter_file(char converted_power_spectrum_filename[], char NGenIC_param_filename[], char filebase2[], int part, double boxsize, double OBh2, double OCh2, double OM, double OL, double OK, double w0, double wa, double ns, double s8, double h, double z, int seed, int power_spectrum_at_zini, double Dplus, double vel_prefac_lam)
{
	...
	fprintf(param_file, "OutputDir        %s/%s/%s/%s                 %%/home/volker/ics/   %% Directory for output\n\n", ics_dir_simcomp, series_folder, Gadget_folder, Gadget_data_folder); // /gpfs/scratch3/jank/Storage/sims/ics/i-series/
...
// Writes parameter file for Gadget-2.
{
	...
	fprintf(param_file, "InitCondFile  	   %s/%s/%s/%s/%s\n", ics_dir_simcomp, series_folder, Gadget_folder, Gadget_data_folder, filebase2);    
...
void write_BGL_description(char NGenIC_param_filename[], char Gadget_param_filename[], char jobstamm[], int job_nr)
{
	...
	fprintf(description_file, "# Classes above are currently: short (1024, 2048, 3072, or 4096 nodes, 12h) , normaldyn: (512 nodes, 24h), long (32 or 128 nodes, 72h).\n");
	fprintf(description_file, "#\n");
