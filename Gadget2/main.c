...
/*
* <JMK>:
* This is a modified version of Volker Springel's original Gadget 2.0.7.
* The modifications include:
* - w(z) dark energy for N-body (not implemented for SPH).
* - Octopus extension, which enables the running of several independent simulations in parallel with one MPI job submission
* - OpenMP shared memory parallelization of the tree force calculation, useful especially for low-memory machines like the IBM Blue Gene/Q.
* - snapshot output file header has been modified.
* The modifications were done by Jan Michael Kratochvil during 2009-2013 for his Inspector Gadget weak lensing simulation pipeline, and are marked with <JMK> … </JMK>.
* </JMK>
*/



// <JMK>: Many modifications in main() below to adapt for Octopus run and mass production with the Inspector Gadget lensing simulation pipeline on the IBM Blue Gene/L, /P, and /Q. Compare to original file to see all changes.
...
int main(int argc, char **argv)
{
  ...
  nSims=atoi(argv[1]); // How many simulations in parallel.
  nProcs=atoi(argv[2]); // On how many processors does each simulation run.

  sim_nr=superrank / nProcs;
  rank_nr= superrank % nProcs;

  MPI_Comm_split(MPI_COMM_WORLD, sim_nr, rank_nr, &sim_comm);

  if (sim_nr>=nSims) // If superfluous process on a CPU, end the process right here.
    {
      MPI_Finalize();
      exit(0);
    }

  MPI_Comm_rank(sim_comm, &ThisTask);
  MPI_Comm_size(sim_comm, &NTask);

  printf("This is Octopus Arm: superrank=%d, supersize=%d, sim_nr=%d, rank_nr=%d, nSims=%d, nProcs=%d, ThisTask(=rank_nr)=%d, NTask(=nProcs)=%d.\n", superrank, supersize, sim_nr, rank_nr, nSims, nProcs, ThisTask, NTask);
  ...
  strcpy(ParameterFile, argv[3+sim_nr]);

  if(argc >= nSims+4)
    {
      RestartFlag = atoi(argv[nSims+3]); // Restart flag comes at the very end of the list of parameter files.
      All.Restart_timer=1; // <JMK>:  Set a flag to start the run timer even if not starting at Step 0.
    }
  else
    {
      RestartFlag = 0;
