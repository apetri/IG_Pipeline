...
void run(void)
{
  ...
  do				/* main loop */
      ...
      if(ThisTask == 0)
	{
      ...
      MPI_Bcast(&stopflag, 1, MPI_INT, 0, sim_comm);
...
void find_next_sync_point_and_drift(void)
{
  ...
  MPI_Allreduce(&min, &min_glob, 1, MPI_INT, MPI_MIN, sim_comm);
  ...
  MPI_Allreduce(&flag, &Flag_FullStep, 1, MPI_INT, MPI_MIN, sim_comm);
  ...
  MPI_Allgather(&NumForceUpdate, 1, MPI_INT, temp, 1, MPI_INT, sim_comm);
  for(n = 0; n < NTask; n++)
...
void every_timestep_stuff(void)
{
  ...
  //<JMK>
  clock_t time_variable;
  float time_ratio, uhr;
  double mpi_timer;

  time_ratio=1./CLOCKS_PER_SEC;
  //</JMK>
      ...
      if(All.ComovingIntegrationOn)
	{
	  ...
	  printf("\nBegin Step %d, Time: %g, Redshift: %g, Systemstep: %g, Dloga: %g\n", All.NumCurrentTiStep,
		 All.Time, z, All.TimeStep, log(All.Time) - log(All.Time - All.TimeStep));
          ...
          //<JMK>:
          time_variable=clock();
          uhr=time_ratio*(long)time_variable;
          printf("Time (in sec since beginning of run): %e\n", uhr);

          mpi_timer=MPI_Wtime();
          if (All.NumCurrentTiStep==0) reference_time=mpi_timer;
	  if (All.Restart_timer==1)
	    {
	      reference_time=mpi_timer;
	      All.Restart_timer=0;
	    }

          mpi_timer=mpi_timer-reference_time;
          printf("MPI-WallTime (in sec since Step 0): %e\n", mpi_timer);
          //</JMK>
