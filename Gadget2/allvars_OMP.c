...
MPI_Comm sim_comm; // new sim-colored communicator for each of the parallel independent sims, replaces MPI_COMM_WORLD in original Gadget-2.
int superrank; // total rank of process in global communicator MPI_COMM_WORLD.
int supersize; // total number of MPI processes requested (includes processes from all sims plus any superfluous ones).
int nProcs; // number of processors on which each simulation is run.
int nSims; // number of simulations run in parallel.
int sim_nr; // simulation number (color of communicator sim_comm).
int rank_nr; // rank of process in individual simulation (rank in MPI communicator sim_comm) same as NTask, used by Octopus wrapper for split of sims.

char *Exportflag_OMP; // Exportflag for threaded OpenMP version.
// </JMK>
