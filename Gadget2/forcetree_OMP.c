...
void force_exchange_pseudodata(void)
{
  ...
  for(level = 1; level < (1 << PTask); level++)
	...
	MPI_Sendrecv(&DomainMoment[DomainStartList[sendTask]],
		     ...
		     MPI_BYTE, recvTask, TAG_DMOM, sim_comm, &status);
    }
...
void force_update_len(void)
{
  ...
  for(level = 1; level < (1 << PTask); level++)
	...
	MPI_Sendrecv(&DomainTreeNodeLen[DomainStartList[sendTask]],
		     ...
		     MPI_BYTE, recvTask, TAG_NODELEN, sim_comm, &status);
    }
...
void force_update_hmax(void)
{
  ...
  for(level = 1; level < (1 << PTask); level++)
	...
	MPI_Sendrecv(&DomainHmax[DomainStartList[sendTask]],
		     ...
		     (DomainEndList[recvTask] - DomainStartList[recvTask] + 1) * sizeof(FLOAT),
		     MPI_BYTE, recvTask, TAG_HMAX, sim_comm, &status);
    }
...
// <JMK>
// Modified function below for OpenMP
...
int force_treeevaluate_shortrange_OMP(int target, int mode, int tid)
{
  struct NODE *nop = 0;
      ...
      else			/* we have an  internal node */
	{
	  if(no >= All.MaxPart + MaxNodes)	/* pseudo particle */
	      ...
	      if(mode == 0)
		{
		  // <JMK>:
		  // Exportflag[DomainTask[no - (All.MaxPart + MaxNodes)]] = 1;
		  Exportflag_OMP[tid*NTask+DomainTask[no - (All.MaxPart + MaxNodes)]] = 1;
		  // </JMK>
		}
	      no = Nextnode[no - MaxNodes];
	      continue;
...
#ifdef DOUBLEPRECISION
	  MPI_Bcast(&fcorrx[0][0][beg], len, MPI_DOUBLE, task, sim_comm);
	  MPI_Bcast(&fcorry[0][0][beg], len, MPI_DOUBLE, task, sim_comm);
	  MPI_Bcast(&fcorrz[0][0][beg], len, MPI_DOUBLE, task, sim_comm);
	  MPI_Bcast(&potcorr[0][0][beg], len, MPI_DOUBLE, task, sim_comm);
#else
	  MPI_Bcast(&fcorrx[0][0][beg], len, MPI_FLOAT, task, sim_comm);
	  MPI_Bcast(&fcorry[0][0][beg], len, MPI_FLOAT, task, sim_comm);
	  MPI_Bcast(&fcorrz[0][0][beg], len, MPI_FLOAT, task, sim_comm);
	  MPI_Bcast(&potcorr[0][0][beg], len, MPI_FLOAT, task, sim_comm);
#endif
	}
