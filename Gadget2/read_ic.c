...
#define SKIP2  {my_fread(&blksize2,sizeof(int),1,fd);}
  ...
  if(ThisTask == readTask)
	...
	MPI_Ssend(&header, sizeof(header), MPI_BYTE, task, TAG_HEADER, sim_comm);
    }
  else
    MPI_Recv(&header, sizeof(header), MPI_BYTE, readTask, TAG_HEADER, sim_comm, &status);
...
int find_files(char *fname)
{
  ...
  MPI_Bcast(&header, sizeof(header), MPI_BYTE, 0, sim_comm);
