...
void savepositions(int num)
{
  ...
  MPI_Allgather(n_type, 6, MPI_INT, temp, 6, MPI_INT, sim_comm);
  for(i = 0; i < 6; i++)
    {
      ntot_type_all[i] = 0;
      for(j = 0; j < NTask; j++)
...
#define SKIP  {my_fwrite(&blksize,sizeof(int),1,fd);}
  ...
  if(ThisTask == writeTask)
	...
	{
	  MPI_Recv(&nn[0], 6, MPI_INT, task, TAG_LOCALN, sim_comm, &status);
	  for(n = 0; n < 6; n++)
	    ntot_type[n] += nn[n];
	}

      for(task = writeTask + 1; task <= lastTask; task++)
	MPI_Send(&ntot_type[0], 6, MPI_INT, task, TAG_N, sim_comm);
    }
  else
    {
      MPI_Send(&n_type[0], 6, MPI_INT, writeTask, TAG_LOCALN, sim_comm);
      MPI_Recv(&ntot_type[0], 6, MPI_INT, writeTask, TAG_N, sim_comm, &status);
    }
  ...
  header.w0 = All.w0;
  header.wa = All.wa;
  header.comoving_distance = calculate_comoving_distance(All.Time);
  header.nothing = 0; // initialize meaningless safety integer in header to zero.
  // </JMK>.
		  ...
		  if(typelist[type])
		      ...
		      for(task = writeTask, offset = 0; task <= lastTask; task++)
			{
			  if(task == ThisTask)
				...
				if(p != ThisTask)
				  MPI_Send(&n_for_this_task, 1, MPI_INT, p, TAG_NFORTHISTASK, sim_comm);
			    }
			  else
			    MPI_Recv(&n_for_this_task, 1, MPI_INT, task, TAG_NFORTHISTASK, sim_comm,
				     &status);
