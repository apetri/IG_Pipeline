...
#define  MAXLEN_FILENAME  500  // <JMK>, original: 100    /*!< Maximum number of characters for filenames (including the full path) */
...
extern MPI_Comm sim_comm; // new sim-colored communicator for each of the parallel independent sims, replaces MPI_COMM_WORLD in original Gadget-2.
extern int superrank; // total rank of process in global communicator MPI_COMM_WORLD.
extern int supersize; // total number of MPI processes requested (includes processes from all sims plus any supefluous ones). 
extern int nProcs; // number of processors on which each simulation is run.
extern int nSims; // number of simulations run in parallel.
extern int sim_nr; // simulation number (color of communicator sim_comm).
extern int rank_nr; // rank of process in individual simulation (rank in MPI communicator sim_comm) same as NTask, used by Octopus wrapper for split of sims.

extern char *Exportflag_OMP; // For threaded OpenMP version.
// </JMK>. 
...
extern struct global_data_all_processes
{
  ...
  // <JMK>:
  int Restart_timer; // flag to restart timer if simulation restarted.
