...
void pm_init_periodic(void)
{
  ...
  fft_forward_plan = rfftw3d_mpi_create_plan(sim_comm, PMGRID, PMGRID, PMGRID,
					     FFTW_REAL_TO_COMPLEX, FFTW_ESTIMATE | FFTW_IN_PLACE);
  fft_inverse_plan = rfftw3d_mpi_create_plan(sim_comm, PMGRID, PMGRID, PMGRID,
  ...
  MPI_Allreduce(slab_to_task_local, slab_to_task, PMGRID, MPI_INT, MPI_SUM, sim_comm);

  MPI_Allreduce(&nslab_x, &smallest_slab, 1, MPI_INT, MPI_MIN, sim_comm);

  slabs_per_task = malloc(NTask * sizeof(int));
  MPI_Allgather(&nslab_x, 1, MPI_INT, slabs_per_task, 1, MPI_INT, sim_comm);
  ...
  MPI_Allgather(&slabstart_x, 1, MPI_INT, first_slab_of_task, 1, MPI_INT, sim_comm);
  ...
  MPI_Allreduce(&fftsize, &maxfftsize, 1, MPI_INT, MPI_MAX, sim_comm);
}
...
void pm_init_periodic_allocate(int dimprod)
{
  ...
  MPI_Allreduce(&dimprod, &dimprodmax, 1, MPI_INT, MPI_MAX, sim_comm);
...
void pmforce_periodic(void)
{
  ...
  // <JMK>
  // Start timing of pm force computation:
  double pm_start_time, pm_run_time;
  pm_start_time=MPI_Wtime();
  // </JMK>
  ...
  MPI_Allgather(meshmin, 3, MPI_INT, meshmin_list, 3, MPI_INT, sim_comm);
  MPI_Allgather(meshmax, 3, MPI_INT, meshmax_list, 3, MPI_INT, sim_comm);
  ...
  // <JMK>:
  double pm_allocate_start_time, pm_allocate_run_time;
  pm_allocate_start_time=MPI_Wtime();
  // </JMK>
  ...
  // <JMK>
  pm_allocate_run_time=MPI_Wtime();
  pm_allocate_run_time-=pm_allocate_start_time;
  if (ThisTask==0) printf("MPI-WallTime (in sec) for PM allocation: %e\n", pm_allocate_run_time);
  // </JMK>
  

  // <JMK>

  double workspace_filling_start_time, workspace_filling_run_time;
  workspace_filling_start_time=MPI_Wtime();

  int max_threads, tid;
  omp_set_num_threads(NR_OF_OMP_THREADS);
  max_threads=omp_get_max_threads();
  if (ThisTask==0) printf("Max OpenMP threads for workspace filling = %d\n", max_threads);

  double fill_workspace[8];

  
#pragma omp parallel num_threads(NR_OF_OMP_THREADS) shared(NumPart,meshmin,to_slab_fac) private(i,tid,slab_x,slab_xx,slab_y,slab_yy,slab_z,slab_zz,dx,dy,dz,fill_workspace)
  ...
  { // OpenMP parallelization begins.                                                   
	...
	printf("Number of threads for workspace filing = %d\n", nthreads);
      }
...
#pragma omp for schedule(static)
  ...
  for(i = 0; i < NumPart; i++)
	...
	slab_z = PMGRID - 1;
      ...
      */
      ...
      fill_workspace[0]= P[i].Mass * (1.0 - dx) * (1.0 - dy) * (1.0 - dz);
      fill_workspace[1]= P[i].Mass * (1.0 - dx) * dy * (1.0 - dz);
      fill_workspace[2]= P[i].Mass * (1.0 - dx) * (1.0 - dy) * dz;
      fill_workspace[3]= P[i].Mass * (1.0 - dx) * dy * dz;

      fill_workspace[4]= P[i].Mass * (dx) * (1.0 - dy) * (1.0 - dz);
      fill_workspace[5]= P[i].Mass * (dx) * dy * (1.0 - dz);
      fill_workspace[6]= P[i].Mass * (dx) * (1.0 - dy) * dz;
      fill_workspace[7]= P[i].Mass * (dx) * dy * dz;

      #pragma omp critical (workspacefill)
      {
	workspace[(slab_x * dimy + slab_y) * dimz + slab_z] +=fill_workspace[0];
	workspace[(slab_x * dimy + slab_yy) * dimz + slab_z] +=fill_workspace[1];
	workspace[(slab_x * dimy + slab_y) * dimz + slab_zz] +=fill_workspace[2];
	workspace[(slab_x * dimy + slab_yy) * dimz + slab_zz] +=fill_workspace[3];

	workspace[(slab_xx * dimy + slab_y) * dimz + slab_z] +=fill_workspace[4];
	workspace[(slab_xx * dimy + slab_yy) * dimz + slab_z] +=fill_workspace[5];
	workspace[(slab_xx * dimy + slab_y) * dimz + slab_zz] +=fill_workspace[6];
	workspace[(slab_xx * dimy + slab_yy) * dimz + slab_zz] +=fill_workspace[7];
      }


    }
  // <JMK>
  } // end of OpenMP parallelization section

  workspace_filling_run_time=MPI_Wtime();
  workspace_filling_run_time-=workspace_filling_start_time;
  if (ThisTask==0) printf("MPI-WallTime (in sec since tree_start_time) for workspace filling: %e\n", workspace_filling_run_time);
	  ...
	  if((recvmax - recvmin) >= 0 || (sendmax - sendmin) >= 0)	/* ok, we have a contribution to the slab */
	      ...
	      if(level > 0)
		{
		  MPI_Sendrecv(workspace + (sendmin - meshmin[0]) * dimy * dimz,
			       ...
			       recvTask, TAG_PERIODIC_A, sim_comm, &status);
		}
	      else
		{
	      ...
	      // <JMK>
	      double fill_rhogrid=0.0;
	      int rhogrid_index;

	      // #pragma omp parallel num_threads(1) shared(rhogrid,forcegrid,first_slab_of_task,ThisTask,slabs_per_task,meshmin_list,meshmax_list,recvTask,recvmin,recv_dimy,recv_dimz) private(slab_x,slab_xx,slab_y,slab_yy,slab_z,slab_zz,fill_rhogrid,rhogrid_index)
	      ...
	      for(slab_x = recvmin; slab_x <= recvmax; slab_x++)
		{
			  ...
			  for(slab_z = meshmin_list[3 * recvTask + 2];
				...
				forcegrid[((slab_x - recvmin) * recv_dimy +
					  ...
					  (slab_z - meshmin_list[3 * recvTask + 2])];
                              ...
                              /*
			      // <JMK>:
                              fill_rhogrid = forcegrid[((slab_x - recvmin) * recv_dimy +
                                           (slab_y - meshmin_list[3 * recvTask + 1])) * recv_dimz +
                                          (slab_z - meshmin_list[3 * recvTask + 2])];
			      rhogrid_index=PMGRID * PMGRID2 * slab_xx + PMGRID2 * slab_yy + slab_zz;

                              #pragma omp critical (rhogrid)
			      rhogrid[rhogrid_index] += fill_rhogrid;
			      // </JMK>.
			      */
			    }
			}
	      ...
	      // } // End OpenMP parallel section </JMK>.
	    }
	}
    }
  ...
  workspace_filling_run_time=MPI_Wtime();
  workspace_filling_run_time-=workspace_filling_start_time;
  if (ThisTask==0) printf("MPI-WallTime (in sec since tree_start_time) for workspace filling post-communication: %e\n", workspace_filling_run_time);
  // </JMK>

  /* Do the FFT of the density field */

  // <JMK>:
  double fft_start_time, fft_run_time;
  fft_start_time=MPI_Wtime();
  // </JMK>

  rfftwnd_mpi(fft_forward_plan, 1, rhogrid, workspace, FFTW_TRANSPOSED_ORDER);

  // <JMK>
  fft_run_time=MPI_Wtime();
  fft_run_time-=fft_start_time;
  if (ThisTask==0) printf("MPI-WallTime (in sec) for FFTW_forward of density field only: %e\n", fft_run_time);
  // </JMK>   
  ...
  // <JMK>:
  double greens_start_time, greens_run_time;
  greens_start_time=MPI_Wtime();

#pragma omp parallel num_threads(NR_OF_OMP_THREADS) shared(slabstart_y,nslab_y,fft_of_rhogrid,asmth2) private(x,y,z,kx,ky,kz,k2,smth,fx,fy,fz,ff,ip)
  // </JMK>
  { // Begin OpenMP parallelization section.
...
#pragma omp for schedule(static)
      ...
      for(z = 0; z < PMGRID / 2 + 1; z++)
	{
	  ...
	  if(k2 > 0)
	      ...
	      if(kz != 0)
		{
	      ...
	      // <JMK>
	      // Defining critical block here is probably not necessary, because ip is distinct for every case (every element of fft_of_rhogrid is accessed only once).
	      // #pragma omp critical (fft_of_rhogrid_fill)
	      {
  ...
  } // end of OpenMP parallel section </JMK>

  if(slabstart_y == 0)
    fft_of_rhogrid[0].re = fft_of_rhogrid[0].im = 0.0;

  // <JMK>                                                                                    
  greens_run_time=MPI_Wtime();
  greens_run_time-=greens_start_time;
  if (ThisTask==0) printf("MPI-WallTime (in sec) for Greens function: %e\n", greens_run_time);
  // </JMK>
  ...
  fft_run_time=MPI_Wtime();
  fft_run_time-=fft_start_time;
  if (ThisTask==0) printf("MPI-WallTime (in sec) for FFTW_backward to get potential: %e\n", fft_run_time);
  // </JMK> 
  ...
  // <JMK>
  double potential_start_time, potential_run_time;
  potential_start_time=MPI_Wtime();
  // </JMK> 
		  ...
		  if(level > 0)
		    {
		      MPI_Sendrecv(forcegrid,
				   ...
				   recvTask, TAG_PERIODIC_B, sim_comm, &status);
		    }
		  else
		    {
  ...
  // <JMK>
  potential_run_time=MPI_Wtime();
  potential_run_time-=potential_start_time;
  if (ThisTask==0) printf("MPI-WallTime (in sec since tree_start_time) for potential compu\
tation: %e\n", potential_run_time);
  // </JMK>


  // <JMK>
  double force_start_time, force_run_time;
  force_start_time=MPI_Wtime();
  // </JMK>
...
#pragma omp parallel num_threads(NR_OF_OMP_THREADS) shared(meshmax,meshmin,forcegrid,dim,dimx,dimy,dimz,fac,recv_dimy,recv_dimz) private(x,y,z,xrr,xll,xr,xl,yrr,yll,yr,yl,zrr,zll,zr,zl)
      {
#pragma omp for schedule(static)
...
#pragma omp parallel num_threads(NR_OF_OMP_THREADS) shared(to_slab_fac,meshmin,meshmax,P,forcegrid,dim,dimx,dimy,dimz) private(slab_x,slab_xx,slab_y,slab_yy,slab_z,slab_zz,dx,dy,dz,acc_dim)
      {
#pragma omp for schedule(static)
      for(i = 0; i < NumPart; i++)
	{
  ...
  // <JMK>
  force_run_time=MPI_Wtime();
  force_run_time-=force_start_time;
  if (ThisTask==0) printf("MPI-WallTime (in sec since tree_start_time) for pm force calculation: %e\n", force_run_time);
  // </JMK>
  ...
  // <JMK>
  // Write out time for pm force computation:
  pm_run_time=MPI_Wtime();
  pm_run_time-=pm_start_time;
  if (ThisTask==0) printf("MPI-WallTime (in sec since pm_start_time) for entire pm force computation: %e\n", pm_run_time);
  // </JMK>
	  ...
	  if((recvmax - recvmin) >= 0 || (sendmax - sendmin) >= 0)	/* ok, we have a contribution to the slab */
	      ...
	      if(level > 0)
		{
		  MPI_Sendrecv(workspace + (sendmin - meshmin[0]) * dimy * dimz,
			       ...
			       (recvmax - recvmin + 1) * recv_dimy * recv_dimz * sizeof(fftw_real), MPI_BYTE,
			       recvTask, TAG_PERIODIC_C, sim_comm, &status);
		}
	      else
		{
		  memcpy(forcegrid, workspace + (sendmin - meshmin[0]) * dimy * dimz,
		  ...
		  if(level > 0)
		    {
		      MPI_Sendrecv(forcegrid,
				   ...
				   workspace + (recvmin - (meshmin[0] - 2)) * dimy * dimz,
				   (recvmax - recvmin + 1) * dimy * dimz * sizeof(fftw_real), MPI_BYTE,
				   recvTask, TAG_PERIODIC_D, sim_comm, &status);
		    }
		  else
		    {
		      memcpy(workspace + (recvmin - (meshmin[0] - 2)) * dimy * dimz,
