...
void domain_Decomposition(void)
{
  ...
  if(All.NumForcesSinceLastDomainDecomp > All.TotNumPart * All.TreeDomainUpdateFrequency)
      ...
      if(ThisTask == 0)
	{
      ...
      MPI_Allgather(&NumPart, 1, MPI_INT, list_NumPart, 1, MPI_INT, sim_comm);
      MPI_Allgather(&N_gas, 1, MPI_INT, list_N_gas, 1, MPI_INT, sim_comm);
...
void domain_decompose(void)
{
  ...
  MPI_Allgather(NtypeLocal, 6, MPI_INT, temp, 6, MPI_INT, sim_comm);
  for(i = 0; i < 6; i++)
    {
      Ntype[i] = 0;
      for(j = 0; j < NTask; j++)
...
void domain_countToGo(void)
{
  ...
  MPI_Allgather(local_toGo, NTask, MPI_INT, toGo, NTask, MPI_INT, sim_comm);
  MPI_Allgather(local_toGoSph, NTask, MPI_INT, toGoSph, NTask, MPI_INT, sim_comm);
}
...
void domain_sumCost(void)
{
  ...
  MPI_Allreduce(local_DomainWork, DomainWork, NTopleaves, MPI_DOUBLE, MPI_SUM, sim_comm);
  MPI_Allreduce(local_DomainCount, DomainCount, NTopleaves, MPI_INT, MPI_SUM, sim_comm);
  MPI_Allreduce(local_DomainCountSph, DomainCountSph, NTopleaves, MPI_INT, MPI_SUM, sim_comm);
...
void domain_findExtent(void)
{
  ...
  MPI_Allreduce(xmin, xmin_glob, 3, MPI_DOUBLE, MPI_MIN, sim_comm);
  MPI_Allreduce(xmax, xmax_glob, 3, MPI_DOUBLE, MPI_MAX, sim_comm);
...
void domain_determineTopTree(void)
{
  ...
  MPI_Allgather(&ntop_local, 1, MPI_INT, ntopnodelist, 1, MPI_INT, sim_comm);
  ...
  if(!(toplist = malloc(ntop * sizeof(struct topnode_exchange))))
    {
      printf("ERROR: Tried to allocate toplist above MPI_Allgatherv in domain.c, but memory allocation failed.\n ntop=%d, sizeof=%d, total=%e.\nAborting run.\n", ((int) ntop), ((int) sizeof(struct topnode_exchange)), ((double) ntop * sizeof(struct topnode_exchange)));
      endrun(11231);
    }
  // </JMK>
  ...
  MPI_Allgatherv(toplist_local, ntop_local * sizeof(struct topnode_exchange), MPI_BYTE,
		 toplist, ntopnodelist, ntopoffset, MPI_BYTE, sim_comm);
