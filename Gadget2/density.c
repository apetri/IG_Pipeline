...
void density(void)
{
  ...
  MPI_Allgather(&NumSphUpdate, 1, MPI_INT, numlist, 1, MPI_INT, sim_comm);
  ...
  do
		  ...
		  if(recvTask < NTask)
		      ...
		      if(nsend[ThisTask * NTask + recvTask] > 0 || nsend[recvTask * NTask + ThisTask] > 0)
			{
			  ...
			  MPI_Sendrecv(&DensDataIn[noffset[recvTask]],
				       ...
				       MPI_BYTE, recvTask, TAG_DENS_A, sim_comm, &status);
			}
		    }
		  ...
		  if(recvTask < NTask)
		      ...
		      if(nsend[ThisTask * NTask + recvTask] > 0 || nsend[recvTask * NTask + ThisTask] > 0)
			{
			  ...
			  MPI_Sendrecv(&DensDataResult[nbuffer[ThisTask]],
				       ...
				       MPI_BYTE, recvTask, TAG_DENS_B, sim_comm, &status);
      ...
      for(i = 0, npleft = 0; i < N_gas; i++)
	{
      ...
      numlist = malloc(NTask * sizeof(int) * NTask);
      MPI_Allgather(&npleft, 1, MPI_INT, numlist, 1, MPI_INT, sim_comm);
      for(i = 0, ntot = 0; i < NTask; i++)
	ntot += numlist[i];
      free(numlist);
  ...
  MPI_Reduce(&timengb, &sumtimengb, 1, MPI_DOUBLE, MPI_SUM, 0, sim_comm);
  MPI_Reduce(&timecomp, &sumt, 1, MPI_DOUBLE, MPI_SUM, 0, sim_comm);
  MPI_Reduce(&timecommsumm, &sumcomm, 1, MPI_DOUBLE, MPI_SUM, 0, sim_comm);
