...
void gravity_forcetest(void)
{
  ...
  MPI_Allreduce(&NumForceUpdate, &ntot, 1, MPI_INT, MPI_SUM, sim_comm);
  ...
  while(ntotleft > 0)
      ...
      for(level = 1; level < (1 << PTask); level++)
	{
		  ...
		  if(nsend[ThisTask * NTask + recvTask] > 0 || nsend[recvTask * NTask + ThisTask] > 0)
		    {
		      ...
		      MPI_Sendrecv(&GravDataIn[noffset[recvTask]],
				   ...
				   recvTask, TAG_DIRECT_A, sim_comm, &status);
		    }
		}
		  ...
		  if(nsend[ThisTask * NTask + recvTask] > 0 || nsend[recvTask * NTask + ThisTask] > 0)
		    {
		      ...
		      MPI_Sendrecv(&GravDataResult[nbuffer[ThisTask]],
				   ...
				   MPI_BYTE, recvTask, TAG_DIRECT_B, sim_comm, &status);
      ...
      MPI_Allreduce(&ndone, &ndonetot, 1, MPI_INT, MPI_SUM, sim_comm);
