...
void pm_init_regionsize(void)
{
  ...
  MPI_Allreduce(xmin, All.Xmintot, 6, MPI_DOUBLE, MPI_MIN, sim_comm);
  MPI_Allreduce(xmax, All.Xmaxtot, 6, MPI_DOUBLE, MPI_MAX, sim_comm);
...
void pm_init_nonperiodic(void)
{
  ...
  fft_forward_plan = rfftw3d_mpi_create_plan(sim_comm, GRID, GRID, GRID,
					     FFTW_REAL_TO_COMPLEX, FFTW_ESTIMATE | FFTW_IN_PLACE);
  fft_inverse_plan = rfftw3d_mpi_create_plan(sim_comm, GRID, GRID, GRID,
  ...
  MPI_Allreduce(slab_to_task_local, slab_to_task, GRID, MPI_INT, MPI_SUM, sim_comm);
...
int pmforce_nonperiodic(int grnr)
{
  ...
  MPI_Allreduce(&flag, &flagsum, 1, MPI_INT, MPI_SUM, sim_comm);
  if(flagsum > 0)
    {
      if(ThisTask == 0)
	{
	  ...
	  if((recvmax - recvmin) >= 0 || (sendmax - sendmin) >= 0)	/* ok, we have a contribution to the slab */
	      ...
	      if(level > 0)
		{
		  MPI_Sendrecv(workspace + (sendmin - meshmin[0]) * dimy * dimz,
			       ...
			       recvTask, TAG_NONPERIOD_A, sim_comm, &status);
		}
	      else
		{
	  ...
	  if((recvmax - recvmin) >= 0 || (sendmax - sendmin) >= 0)	/* ok, we have a contribution to the slab */
	      ...
	      if(level > 0)
		{
		  MPI_Sendrecv(forcegrid,
			       ...
			       recvTask, TAG_NONPERIOD_B, sim_comm, &status);
		}
	      else
		{
	  ...
	  if((recvmax - recvmin) >= 0 || (sendmax - sendmin) >= 0)	/* ok, we have a contribution to the slab */
	      ...
	      if(level > 0)
		{
		  MPI_Sendrecv(workspace + (sendmin - meshmin[0]) * dimy * dimz,
			       ...
			       (recvmax - recvmin + 1) * recv_dimy * recv_dimz * sizeof(fftw_real), MPI_BYTE,
			       recvTask, TAG_NONPERIOD_C, sim_comm, &status);
		}
	      else
		{
		  memcpy(forcegrid, workspace + (sendmin - meshmin[0]) * dimy * dimz,
	  ...
	  if((recvmax - recvmin) >= 0 || (sendmax - sendmin) >= 0)	/* ok, we have a contribution to the slab */
	      ...
	      if(level > 0)
		{
		  MPI_Sendrecv(forcegrid,
			       ...
			       workspace + (recvmin - (meshmin[0] - 2)) * dimy * dimz,
			       (recvmax - recvmin + 1) * dimy * dimz * sizeof(fftw_real), MPI_BYTE,
			       recvTask, TAG_NONPERIOD_D, sim_comm, &status);
		}
	      else
		{
		  memcpy(workspace + (recvmin - (meshmin[0] - 2)) * dimy * dimz,
