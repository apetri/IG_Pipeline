...
//</JMK>
  ...
  MPI_Allreduce(&dispmax, &globmax, 1, MPI_DOUBLE, MPI_MAX, sim_comm);
  MPI_Allreduce(&disp2sum, &globdisp2sum, 1, MPI_DOUBLE, MPI_SUM, sim_comm);
  ...
  MPI_Allreduce(&ti_step, &All.PresentMinStep, 1, MPI_INT, MPI_MIN, sim_comm);
...
void find_dt_displacement_constraint(double hfac /*!<  should be  a^2*H(a)  */ )
{
  ...
  if(All.ComovingIntegrationOn)
      ...
      for(i = 0; i < NumPart; i++)
	{
      ...
      MPI_Allreduce(v, v_sum, 6, MPI_DOUBLE, MPI_SUM, sim_comm);
      MPI_Allreduce(mim, min_mass, 6, MPI_DOUBLE, MPI_MIN, sim_comm);

      temp = malloc(NTask * 6 * sizeof(int));
      MPI_Allgather(count, 6, MPI_INT, temp, 6, MPI_INT, sim_comm);
      for(i = 0; i < 6; i++)
	{
	  count_sum[i] = 0;
	  for(j = 0; j < NTask; j++)
