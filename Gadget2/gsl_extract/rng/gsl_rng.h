...
// <JMK>:
#include "../config.h"
// WARNING: The above is a necessary addition here, since in order to use gsl_inline.h and gsl_rng_uniform() here, must have defined HAVE_INLINE, which is in config.h. On computer systems which do not have inline, caution is advised, since I do not know how gsl_rng_uniform() is implemented in GSL for those (it is easy to just comment out the if-statement checking for inline on those systems to still use the same function, but if the code then still behaves correctly, I do not know.
// </JMK>
