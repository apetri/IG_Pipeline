  ...
  for(groupTask = 0; groupTask < nprocgroup; groupTask++)
      ...
      if(ThisTask == (masterTask + groupTask))	/* ok, it's this processor's turn */
	{
	  if(modus)
	    ...
	    {
          // <JMK>: Removed the line below, because Blue Gene does not like system calls.
	      // system(buf_mv);	/* move old restart files to .bak files */

          // Note: Disadvantage of removing the above line is that restart files are not backed up before they are overwritten, so a run crash during restart file writing makes the run non-recoverable. (This is only a serious disadvantage for large simulations which take many days to run, because otherwise it is not a problem to rerun the simulation from the beginning, which is advisable anyway after an unexpected crash has occurred.
          // </JMK>
	  ...
	  if(modus > 0 && groupTask == 0)	/* read */
	    {
	      MPI_Bcast(&all_task0, sizeof(struct global_data_all_processes), MPI_BYTE, 0, sim_comm);
	    }
      ...
      in(&NTopleaves, modus);
      // <JMK>: The one line above (NTopleaves) is a new addition in Gadget 2.0.7 compared to Gadget 2.0.3.
