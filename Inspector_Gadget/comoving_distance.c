...
void derivs_c (double x, double y[], double dydx[])
{
	...
	dydx[1]=1.0/(sqrt(parameters.Omega_m*(1.0+x)*(1.0+x)*(1.0+x)+parameters.Omega_Lambda*DarkEnergy(1.0/(1.0+x)))); // arbitrary examples of first order differential equations being solved (this line and the next two).
}
...
double calculate_comoving_distance(double time) {
	...
	if (parameters.darkenergy_initialized!=1)
	  {
	    printf("ERROR: Dark energy not initialized before calling calculate_comoving_distance(). Must call initialize_darkenergy() before first use of this function. Aborting.\n");
	...
	//	printf("Superrank: %d before free, Comoving distance direct: %e, kount_c %d, speedoflight %e, DEp_c[1][kount_c] %e\n", superrank, comoving_distance, kount_c, speedoflight, DEp_c[1][kount_c]);
	...
	//	printf("Finished calculating comoving distance, and dark energy parameters are: Omgea_m=%e, Omega_Lambda=%e, w0=%e, wa=%e.\n", parameters.Omega_m, parameters.Omega_Lambda, parameters.w0, parameters.wa);
